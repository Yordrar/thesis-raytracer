...
/**
 * @brief A function wrapper that checks to see if the final data destination of
 * a storage_mem object is null.
 */
using is_final_data_null_t = std::function<bool()>;

/**
 * @brief A function wrapper that enables a storage_mem object to be copied into
 * a final data destination.
 */
...
class COMPUTECPP_EXPORT final_data {
 public:
  ...
  /**
   * @brief Set the function callback that will be responsible for null checking
   * the final data destination. The function must return true if the data
   * location is null.
   * @param func The function callback.
   * @return A reference to this to enable function chaining.
   */
