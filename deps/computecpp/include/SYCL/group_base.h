...
class group_base {
 ...
 public:
  ...
  /** group_base
   * \brief Constructor: Assigns the local range, global range, number of groups
   * and group id provided, then calculates the linear local range, linear
   * global
   * range and the number of work groups.
   ...
   */
  group_base(detail::index_array groupID, detail::index_array groupRange,
             detail::index_array globalRange, detail::index_array localRange);
...
inline group_base::group_base(detail::index_array groupID,
    ...
    : m_globalRange(globalRange),
      ...
      m_groupID(groupID) {
  m_linearGroupID = detail::construct_linear_row_major_index(
      m_groupID[0], m_groupID[1], m_groupID[2], m_groupRange[0],
      m_groupRange[1], m_groupRange[2]);
}
