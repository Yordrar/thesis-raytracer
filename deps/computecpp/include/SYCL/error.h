...
/*!
  ...
  @brief Internal APIs for error logging
*/
...
/** sycl_log.
 * @brief Container for all information relating to a SYCL log, this can be
 * an error, a warning or just information.
 */
struct COMPUTECPP_EXPORT sycl_log {
  log_type logType;
  string_class fileName;
  int lineNum;
  string_class openclErrorMessage;
  ...
  /* sycl_log.
   * @brief This class can only be defined for an specific error and there
   * is no point in having an empty default constructor.
   */
  sycl_log() = delete;

  /*!
  @brief Constructs a sycl_log wit
  ...
  the log.
  @param fileName The name of the file where the log was triggered.
  @param lineNumber The line number where the log was triggered.
  @param contextPointer An optional pointer to a context associated with the log
  for handling asynchronous errors.
  */
  sycl_log(log_type logType, string_class fileName, int lineNumber,
           cl_int openclErrorCode, detail::cpp_error_code cppErrorCode,
           const detail::context* contextPointer,
           const string_class& userMessage, const char* extraInformation);
};
}  // namespace detail
...
class COMPUTECPP_EXPORT exception {
 public:
   ...
   */
  explicit exception(detail::sycl_log&& syclLog,
   ...
   */
  string_class get_description() const;
...
/**
 * @brief Class used to store exception objects and transfer them across thread,
 * equivalent to std::exception_ptr.
 */
...
namespace detail {
/*!
@brief Triggers a log in the ComputeCpp runtime. This can be of any type of log
specified by the log_type argument. A sycl_log is constructed from the arguments
provided and is then passed on to the handle_sycl_log() function to be
processed.
@param logType The type of log that is being triggered.
@param filePath The path to the file where the log was triggered.
@param lineNumber The line number where the log was triggered.
@param openCLErrorCode An optional OpenCL error code that is associated with the
log.
@param contextPointer An optional pointer to a context associated with the log
for handling asynchronous errors.
@param message The log message.
@param ... Formatting that is associated with the log message.
*/
...
/**
 * @brief Handles the output of the SYCL log based on the log level.
 * @tparam T the
 * @param syclLog The sycl_log to be handled.
 */
template <typename exception_t>
COMPUTECPP_EXPORT void handle_sycl_log(detail::sycl_log&& syclLog);

/**
 * @brief Outputs a a message to the console with the "ComputeCpp>" prefix.
 * @param outputTarget The target for the log to be output, i.e. stdout,
 * stderr, or a file.
 * @param fmt The message to be output to the
 * console.
 */
COMPUTECPP_EXPORT void output_sycl_log(FILE* outputTarget, const char* fmt,
                                       ...);
