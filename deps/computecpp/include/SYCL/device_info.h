...
COMPUTECPP_DEFINE_SYCL_INFO_PARAMETER(device, queue_profiling,
                                      CL_DEVICE_QUEUE_PROPERTIES, cl_bool,
 ...
 */
constexpr const size_t hostMemoryMaxAlloc = (1u << 31);

/**
 * @brief Size of memory available on the host device, in bytes.
 */
constexpr const size_t hostMemorySize = hostMemoryMaxAlloc;
    ...
    switch (*clValue) {
      ...
      default:
        COMPUTECPP_CL_ERROR_CODE_MSG(
            CL_SUCCESS, detail::cpp_error_code::UNREACHABLE_ERROR, nullptr,
...
struct info_convert<size_t*, id<3> > {
  static id<3> cl_to_sycl(size_t* clValue, size_t numElems) {
    if (numElems != 3) {
      COMPUTECPP_CL_ERROR_CODE_MSG(
          CL_SUCCESS, detail::cpp_error_code::UNREACHABLE_ERROR, nullptr,
          "Invalid conversion form size_t * to id<3>")
    }
