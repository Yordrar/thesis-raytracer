...
class item : public detail::item_base {
 ...
 public:
  ...
  /** \brief conversion operator from \ref item to \ref id
   * @return global id for all dimensions
   */
  operator id<dimensions>() const { return id<dimensions>(this->get_id()); }
...
class nd_item : public detail::nd_item_base {
 ...
 protected:
  ...
  /**
   * \brief This constructor should not be called. nd_item has to be constructed
   * from the runtime.
   * \warning Cannot be used (deleted).
   */
  nd_item() = delete;
 ...
 public:
   ...
   * @brief copies the given nd_item with the same dimentionality.
  ...
  /**
   * \brief Retrieves an object of the sub_group class which contains
   * the sub group range
   * \return A new instance of the class sub_group
   */
  codeplay::sub_group get_sub_group() const {
    // using codeplay::sub_group;
#ifdef __SYCL_DEVICE_ONLY__
    using codeplay::sub_group;
    return sub_group(detail::get_sub_group_id(),
                     base_t::get_global_range(0) / detail::get_sub_group_size(),
                     detail::get_sub_group_item_id(),
                     detail::get_sub_group_size());
#else
    return codeplay::sub_group(0, 1, get_local_id(0), 0);
#endif  // __SYCL_DEVICE_ONLY__
  }

  /**
   * \brief Access to subgroup barrier
   * \param access::fence_space
   * \return A barrier or undefined behaviour if (accessSpace !=
   * access::gence_space::global_and_local)
   */
  void sub_group_barrier(access::fence_space accessSpace =
    ...
    COMPUTECPP_NOT_IMPLEMENTED(
        "sub_group_barrier on the host is not implemented")
#endif  // __SYCL_DEVICE_ONLY__
  }
  ...
  COMPUTECPP_DEPRECATED_API(
      "SYCL 1.2.1 revision 3 replaces nd_item::get_global with "
   ...
   */
  COMPUTECPP_DEPRECATED_API(
      "SYCL 1.2.1 revision 3 replaces nd_item::get_local with "
      "nd_item::get_local_id.")
   ...
   */
  COMPUTECPP_DEPRECATED_API(
      "SYCL 1.2.1 revision 3 replaces nd_item::get_num_groups with "
  ...
  */
...
#ifdef __SYCL_DEVICE_ONLY__
    return device_event(
        detail::async_work_group_copy(dest, src, numElements, 0));
  ...
  /**
   * @brief Performs an asynchronous copy from a global memory plane to subgroup
   * local memory.
   * @param source The region of memory to copy data from.
   * @param destination The region of memory to write the data to.
   * @param copyBounds The shape of the region.
  ...
  /**
   * @brief Performs an asynchronous copy from subgroup local memory to a global
   * memory plane.
   * @param source The region of memory to copy data from.
   * @param destination The region of memory to write the data to.
   * @param copyBounds The shape of the region.
