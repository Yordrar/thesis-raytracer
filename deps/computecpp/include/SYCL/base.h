...
class kernel_functor_container {
 public:
  explicit kernel_functor_container(functorT kernelFunction)
      : kernelFunction(kernelFunction), kernelString(0) {}

  kernel_functor_container(functorT kernelFunction, const char* kernelString)
      : kernelFunction(kernelFunction), kernelString(kernelString) {}

  ~kernel_functor_container() {}

  bool has_kernel_string() { return (kernelString != 0); }

  functorT kernelFunction;
...
#define COMPUTECPP_CL_ASP_PRIVATE                                 // __private
#define COMPUTECPP_CL_ASP_GLOBAL __attribute__((ocl_global))      // __global
#define COMPUTECPP_CL_ASP_CONSTANT __attribute__((ocl_constant))  // __constant
#define COMPUTECPP_CL_ASP_LOCAL __attribute__((ocl_local))        // __local
...
namespace detail {
COMPUTECPP_EXPORT void host_barrier(detail::nd_item_base);
