...
class COMPUTECPP_EXPORT image_mem : public storage_mem {
 protected:
  image_mem(string_class error_message) : storage_mem() {
...
class image : public image_mem {
 ...
 public:
   ...
   */
  image(image<kDimensions, AllocatorT>&& rhs)
      : image_mem(std::move(static_cast<image_mem>(rhs))) {}
  ...
  image<kDimensions, AllocatorT>& operator=(
    ...
    this->set_impl(std::move(rhs.get_impl()));
    return *this;
  }
