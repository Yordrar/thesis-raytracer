...
class multi_ptr : public detail::multi_ptr_base<dataType, asp> {
 ...
 public:
   ...
   */
  using reference_t = typename multi_ptr_base::asp_type&;

  /**
   * @brief Raw reference-to-const definition.
   */
  using const_reference_t = const typename multi_ptr_base::asp_type&;

  using element_type = typename multi_ptr_base::element_type;
  using difference_type = typename multi_ptr_base::difference_type;
  using pointer_t = typename multi_ptr_base::pointer_t;
  using const_pointer_t = typename multi_ptr_base::const_pointer_t;
   ...
   * @param ptr Pointer that the class should manipulate.
   */
  multi_ptr(pointer_t ptr) : multi_ptr_base(ptr) {}
   ...
   */
  template <COMPUTECPP_ENABLE_IF(dataType,
                                 (!std::is_same<pointer_t, dataType*>::value))>
   ...
   */
  COMPUTECPP_DEPRECATED_API("No operator[] for multi_ptr in SYCL 1.2.1")
...
class multi_ptr<void, asp> : public detail::multi_ptr_base<void, asp> {
 ...
 public:
  ...
  template <COMPUTECPP_ENABLE_IF(void,
                                 (!std::is_same<pointer_t, void*>::value))>
  multi_ptr(void* ptr);
  ...
  explicit multi_ptr(const multi_ptr<ElementType, asp>& ptr)
      : multi_ptr_base(static_cast<pointer_t>(ptr.get())) {}
...
class multi_ptr<const void, asp>
 ...
 public:
   ...
   *       should never actually be used.
   */
  template <COMPUTECPP_ENABLE_IF(
      const void, (!std::is_same<pointer_t, const void*>::value))>
...
typename std::enable_if<
    !std::is_same<typename multi_ptr<dataType, Space>::pointer_t,
                  dataType*>::value,
