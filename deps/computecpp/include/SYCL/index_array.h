...
inline size_t& index_array::operator[](unsigned int dimension) {
  if (dimension >= 3) {
    COMPUTECPP_CL_ERROR_CODE_MSG(CL_SUCCESS,
                                 detail::cpp_error_code::UNREACHABLE_ERROR,
                                 nullptr, "Incorrect number of dimensions");
  }
...
inline index_array& index_array::operator=(const index_array& rhs) {
  m_idx[0] = rhs.m_idx[0];
  m_idx[1] = rhs.m_idx[1];
  m_idx[2] = rhs.m_idx[2];
  return *this;
...
class index_array_base {
 public:
   ...
   */
  index_array_base& operator=(const index_array_base& rhs);
...
inline index_array_base& index_array_base::operator=(
    const index_array_base& rhs) {
  m_idx = rhs.m_idx;
  return *this;
}
