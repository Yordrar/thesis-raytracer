  ...
  static vector_class<syclType> cl_to_sycl(clType* clPtr, size_t numElems) {
    ...
    for (size_t i = 0; i < numElems; i++) {
      syclVector.push_back(
          info_convert<clType*, syclType>::cl_to_sycl(&(clPtr[i]), 1));
    }
    return syclVector;
  }
};
...
struct COMPUTECPP_EXPORT info_convert<char*, string_class> {
  static string_class cl_to_sycl(char* clPtr, size_t numElems);
};
...
/*!
 * @brief Retrieves the OpenCL info value for the parameter specified by clParam
 *        as the OpenCL type specified by clType and returns it as the
...
syclType get_sycl_info(
  ...
  /* If the requesting object is not in hose mode retrieve the info through the
    ...
    return detail::get_opencl_info_as_sycl<syclInfo, syclType, clType>(clObject,
                                                                       clParam);
  }
}
