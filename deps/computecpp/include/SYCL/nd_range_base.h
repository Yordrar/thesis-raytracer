...
class nd_range_base {
 public:
   ...
   */
  nd_range_base(const nd_range_base& rhs) = default;
  ...
  /**
   * \brief Returns whether or not the specified dimension of the global range
   * is divisible by the correspondent local size
   * @return True if divisible, false if not.
   */
  bool is_divisible(const unsigned& i) const;
...
inline nd_range_base::nd_range_base(  // NOLINT This constructor is used for
  ...
  if ((xss.size() < 1) || (xss.size() > 2)) {
    COMPUTECPP_CL_ERROR_CODE_MSG(
        ...
        "Invalid number of work\
        sizes provided in initializer list")
    return;
  }
    ...
    for (size_t const& val : (*(xss.begin() + sizeIndex))) {
      if (idIndex >= 3) {
        COMPUTECPP_CL_ERROR_CODE_MSG(
            CL_SUCCESS, detail::cpp_error_code::CREATE_NDRANGE_ERROR, nullptr,
            "Invalid number of\
            arguments for ND Range provided in initializer list")
...
inline bool nd_range_base::is_divisible(
    const unsigned int& i) const {  // NOLINT false +
  if ((m_globalRange[i] % m_localRange[i])) {
    return false;
  }
  return true;
}
...
inline detail::index_array nd_range_base::get_group_range() const {
  return detail::index_array(m_globalRange[0] / m_localRange[0],
                             m_globalRange[1] / m_localRange[1],
                             m_globalRange[2] / m_localRange[2]);
}
