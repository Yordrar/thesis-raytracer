...
/** mem_container
* Specialisation of mem_container for recursion size 1. Inherits from
  mem_container_base.
*/
template <typename dataT, int kElems>
...
/** mem_container
* Specialisation of mem_container for recursion size 2. Inherits from
  mem_container for recursion size 1.
*/
template <typename dataT, int kElems>
class mem_container<dataT, kElems, 2> : public mem_container<dataT, kElems, 1> {
 public:
  /* Simple swizzles and single element access methods are defined using the
    COMPUTECPP_DEFINE_SIMPLE_SWIZZLE_N macros. Swizzle methods are only
    implemented if the
    SYCL_SIMPLE_SWIZZLES macro is defined.
        */
...
/** mem_container
* Specialisation of mem_container for recursion size 3. Inherits from
...
/** mem_container
* Specialisation of mem_container for recursion size 4. Inherits from
...
/** mem_container
* Specialisation of mem_container for recursion size 8. Inherits from
  mem_container_base.
*/
template <typename dataT, int kElems>
...
/** mem_container
* Specialisation of mem_container for recursion size 16. Inherits from
...
class vec : public detail::mem_container<dataT, kElems, kElems> {
 public:
  ...
  /** constructor (default)
   * Initialises all vector values to 0.
   */
  vec();

  /** constructor (scalar)
   * Initialises all vector values to the scalar.
   */
  ...
  /** constructor (copy)
   * Initialises each vector value to the respective element of the rhs vec.
   */
  vec(const vec<dataT, kElems>& rhs);

#ifdef __SYCL_DEVICE_ONLY__
  /** constructor (__sycl_vector)
   * Initialises the vector with the __sycl_vector parameter. Device only.
   */
  template <typename vecDataT, typename E = typename std::enable_if<
                                   std::is_same<dataT, vecDataT>::value>::type>
  explicit vec(detail::__sycl_vector<vecDataT, kElems> rhs) {
    this->m_data = rhs;
  }

  /** conversion operator (__sycl_vector)
   * Implicitly converts a vector to the underlying vector type. Device only.
   */
  operator detail::__sycl_vector<dataT, kElems>();

  /** conversion operator (__sycl_vector)
   * Implicitly converts a vector to the underlying vector type. Device only.
   */
  operator detail::__sycl_vector<dataT, kElems>() const;
  ...
  /** operator= (vec)
   * Assigns a vec object to this object.
